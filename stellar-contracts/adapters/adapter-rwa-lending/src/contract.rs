...
/// Adapter connecting rwa-vault to a single rwa-lending pool.
///
...
#[contract]
pub struct RwaLendingAdapter;

#[contractimpl]
impl RwaLendingAdapter {
    // ========== Initialization ==========
    ...
    pub fn a_deposit(env: Env, amount: i128, _from: Address) -> i128 {
        ...
        let lending = rwa_lending::Client::new(&env, &storage.lending_pool);
    ...
    /// Withdraw tokens from rwa-lending and transfer them to `to` (the vault).
    ...
    /// Returns the adapter's current value in rwa-lending expressed in deposit_token units.
    ...
    /// Approximated from b_rate change — for MVP returns 0 (no on-chain rate query in rwa-lending).
    pub fn a_get_apy(_env: Env) -> u32 {
        // TODO: integrate with rwa-lending interest rate model to compute APY
