...
fn create_oracle(e: &Env) -> (neko_oracle::Client<'_>, Address) {
    let asset_nvda = neko_oracle::Asset::Other(Symbol::new(e, "NVDA"));
    let asset_usdc = neko_oracle::Asset::Other(Symbol::new(e, "USDC"));
    ...
    let client = neko_oracle::Client::new(e, &contract_address);
...
fn test_collateral_factor() {
    ...
    let neko_token = Address::generate(&env);
    let factor = 7_500_000; // 75% (7 decimals)

    // Set collateral factor — RWA token uses RWA oracle
    client.set_collateral_factor(&neko_token, &factor, &AssetType::Rwa, &symbol_short!("NVDA"));

    // Get collateral factor
    let retrieved_factor = client.get_collateral_factor(&neko_token);
