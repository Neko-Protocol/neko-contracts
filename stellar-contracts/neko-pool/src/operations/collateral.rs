...
impl Collateral {
    ...
    pub fn add_collateral(
        ...
        let token_client = TokenClient::new(env, neko_token);
        let contract_address = env.current_contract_address();
        token_client.transfer(borrower, &contract_address, &amount);

        // Update collateral balance
        let current_collateral = Storage::get_collateral(env, borrower, neko_token);
        Storage::set_collateral(env, borrower, neko_token, current_collateral + amount);
        ...
        cdp.collateral
            .set(neko_token.clone(), current_collateral + amount);
        cdp.last_update = env.ledger().timestamp();
        Storage::set_cdp(env, borrower, &cdp);

        // Emit event
        Events::add_collateral(env, borrower, neko_token, amount);
    ...
    pub fn remove_collateral(
            ...
            && cdp.d_tokens > 0
        {
            ...
            Storage::set_collateral(env, borrower, neko_token, new_collateral);

            // Temporarily update CDP to calculate new borrow limit
            let mut temp_cdp = cdp.clone();
            temp_cdp.collateral.set(neko_token.clone(), new_collateral);
            ...
            if let Some(debt_asset) = &cdp.debt_asset {
                ...
                Storage::set_cdp(env, borrower, cdp);
                Storage::set_collateral(env, borrower, neko_token, current_collateral);
                ...
                let health_factor =
                    crate::operations::liquidations::Liquidations::calculate_health_factor(
                        env, borrower,
                    )?;
        ...
        // Emit event
        Events::remove_collateral(env, borrower, neko_token, amount);
    ...
    pub fn get_collateral(env: &Env, borrower: &Address, neko_token: &Address) -> i128 {
        Storage::get_collateral(env, borrower, neko_token)
    }
