...
use crate::{RwaLendingAdapter, RwaLendingAdapterClient};

/// Import rwa-oracle WASM for test setup.
...
fn create_rwa_oracle(env: &Env) -> Address {
    let asset_cetes = rwa_oracle::Asset::Other(symbol_short!("CETES"));
    let asset_usdc = rwa_oracle::Asset::Other(symbol_short!("USDC"));
...
fn create_lending_pool(env: &Env, token: &Address, oracle_addr: &Address) -> Address {
    use rwa_lending::AssetType;

    let lending_admin = Address::generate(env);
    let reflector = create_rwa_oracle(env); // second oracle for reflector
    let pool_id = env.register(rwa_lending::WASM, ());
    let pool = rwa_lending::Client::new(env, &pool_id);
    ...
    pool.set_interest_rate_params(
        &CETES(env),
        &rwa_lending::InterestRateParams {
    ...
    // Activate pool
    pool.set_pool_state(&rwa_lending::PoolState::Active);
...
) -> RwaLendingAdapterClient<'a> {
    let contract_id = env.register(RwaLendingAdapter, ());
    let client = RwaLendingAdapterClient::new(env, &contract_id);
...
fn test_adapter_initialize() {
    ...
    let oracle = create_rwa_oracle(&env);
