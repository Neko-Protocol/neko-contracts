...
fn create_neko_oracle_contract<'a>(e: &Env) -> RWAOracleClient<'a> {
...
fn create_neko_oracle_contract_with_admin<'a>(e: &Env, admin: &Address) -> RWAOracleClient<'a> {
    set_ledger_timestamp(e, 2_000_000_000);
    let asset_xlm: Asset = Asset::Other(Symbol::new(e, "NVDA"));
    let asset_usdt: Asset = Asset::Other(Symbol::new(e, "TSLA"));
    let asset_vec = Vec::from_array(e, [asset_xlm.clone(), asset_usdt.clone()]);
...
#[test]
fn test_neko_oracle_initialization() {
    let e = Env::default();
    e.mock_all_auths();
...
fn test_only_admin_can_propose() {
    ...
    let oracle = create_neko_oracle_contract_with_admin(&e, &admin);
