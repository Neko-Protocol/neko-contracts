...
use crate::neko_oracle::{self, Asset, PriceData as OraclePriceData};
...
impl Oracle {
    ...
    pub fn get_price(env: &Env) -> Result<OraclePriceData, Error> {
        ...
        let oracle_client = neko_oracle::Client::new(env, &asset_contract);
    ...
    pub fn get_rwa_metadata(env: &Env) -> Result<neko_oracle::RWAMetadata, Error> {
        let asset_contract = Self::get_asset_contract(env);
        let pegged_asset = Self::get_pegged_asset(env);
        let oracle_client = neko_oracle::Client::new(env, &asset_contract);
    ...
    pub fn get_asset_type(env: &Env) -> Result<neko_oracle::RWAAssetType, Error> {
